...
def kplane_param_count(module):
    # return total MB,space grids MB,space-time grids MB, net MB 
    total = count_params(module)
    space_time = 0
    space = 0
    for grid in module.grids:
        for idx, plane in enumerate(grid):
            if idx!=2 and id!=4 and idx!=5:
                space += plane.numel()
            else:
                space_time += plane.numel()
    non_grid = count_params(module.direction_encoder) + count_params(module.sigma_net) + count_params(module.color_net)
    return (space + space_time), space, space_time, non_grid
...
@torch.no_grad()
def decompose_space_time(trainer, extra_name: str = "") -> None:

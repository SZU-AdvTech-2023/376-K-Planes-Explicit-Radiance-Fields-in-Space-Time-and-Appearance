...
from compress import compress_mask,decompress_mask
...
def init_trainer(model_type: str,use_mask, mask_weight, **kwargs):
    if model_type == "video":
        from plenoxels.runners import video_trainer
        return video_trainer.VideoTrainer(use_mask,mask_weight,**kwargs)
    elif model_type == "phototourism":
        from plenoxels.runners import phototourism_trainer
...
def main():
    ...
    decompress_only = args.decompress_only
    ...
    data = load_data(model_type,
                     ...
                     **config)
    config.update(data)
    trainer = init_trainer(model_type, use_mask = args.use_mask, mask_weight = args.mask_weight, **config)
    if args.log_dir is not None and not decompress_only:
        checkpoint_path = os.path.join(args.log_dir, "model.pth")
        training_needed = not (validate_only or render_only or spacetime_only
                               or compress_only or decompress_only)
    ...
    if validate_only:
        ...
        if args.log_dir is None:
            raise ValueError("compress must be specify model dir path.")
        compress_mask(trainer, args, extra_name="")
    elif decompress_only:
        if args.log_dir is None:
            raise ValueError("decompress must be specify model dir path.")
        decompress_mask(trainer, args, extra_name="")
    else:
        trainer.train(args)

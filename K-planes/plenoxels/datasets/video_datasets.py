...
class Video360Dataset(BaseDataset):
    ...
    def __init__(self,
        ...
        if "lego" in datadir or "D-NeRF_dataset" in datadir:
            dset_type = "synthetic"
        else:
            dset_type = "llff"
        ...
        if split == 'train':
            self.timestamps = self.timestamps[:, None, None].repeat(
                1, intrinsics.height, intrinsics.width).reshape(-1)  # [n_cameras * n_frames * h * w]
    ...
    def __getitem__(self, index):
        ...
        imgs = out['imgs']
        # Decide BG color
        bg_color = torch.ones((1, 3), dtype=torch.float32, device=dev) # orginal
...
def load_llffvideo_data(videopaths: List[str],
    ...
    timestamps = torch.cat(timestamps, 0)  # [N], N = num_cameras* 300frame

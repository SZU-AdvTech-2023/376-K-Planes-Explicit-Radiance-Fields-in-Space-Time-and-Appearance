...
class LowrankModel(nn.Module):
    ...
    def step_before_iter(self, step):
        if self.use_occ_grid and self.training:
            def occ_eval_fn(x):
                ...
                density = self.field.get_density(x[:,None],timestamps=torch.rand_like(x[:, 0])*2 -1 if requires_timestamps else None)[0][:,0]
    ...
    def get_params(self, lr: float):
        if not self.use_occ_grid:
            ...
            if self.field.use_mask:
                field_masks_params = model_params["field_masks"]
            nn_params = model_params["nn"]
            other_params = model_params["other"]
            if self.field.use_mask:
                return [
                    {"params": field_params, "lr": lr},
                    {"params": field_masks_params, "lr": lr},
                    # {"params": field_masks_params, "lr": lr * 0.1},
                    {"params": nn_params, "lr": lr},
                    {"params": other_params, "lr": lr},
                ]
            else:

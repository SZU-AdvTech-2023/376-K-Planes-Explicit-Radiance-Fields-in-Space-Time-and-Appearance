...
from plenoxels.models.kplane_field import min_max_quantize
from utils.create_rendering import kplane_param_count
from plenoxels.utils.s3im import S3IM


class BaseTrainer(abc.ABC):

    def __init__(self, use_mask, mask_weight, train_data_loader: Iterable,
                 num_steps: int, logdir: str, expname: str, train_fp16: bool,
                 save_every: int, valid_every: int, save_outputs: bool,
                 device: Union[str, torch.device], **kwargs):
        ...
        self.model = self.init_model(use_mask, **self.extra_args)
    ...
    def train_step(self, data, **kwargs) -> bool:
        ...
        with torch.cuda.amp.autocast(enabled=self.train_fp16):
            fwd_out = self.model(data['rays_o'],
                                 data['rays_d'],
            ...
            if self.s3im_weight > 0:
                s3im_pp = self.s3im_weight * self.s3im_func(fwd_out['rgb'], data['imgs'])
                loss += s3im_pp
            ...
            if self.model.field.use_mask and self.mask_weight > 0:
                ...
                if self.global_step % 500 == 0:
                    print("loss:",loss.item(), " loss1:",recon_loss.item(), " loss2:",reg_loss.item()," loss3:",(self.mask_weight * mask_loss).item())
    ...
    def post_step(self, progress_bar):
        ...
        if self.global_step % self.calc_metrics_every == 0:
            progress_bar.set_postfix_str(losses_to_postfix(self.loss_info,
                                                           lr=self.lr),
                                         refresh=False)
            for loss_name, loss_val in self.loss_info.items():
                self.writer.add_scalar(f"train/loss/{loss_name}",
                                       loss_val.value, self.global_step)
        ...
        if self.save_every > -1 and self.global_step % self.save_every == 0:
            print()
            if self.model.field.use_mask:
                with torch.no_grad():
                    for res in range(len(self.model.field.grids)):
                        t = 0
                        for i in range(len(self.model.field.grids[res])):
                            if i!=2 and i!=4 and i!=5:
                                self.model.field.grids[res][i].set_(
                                    self.model.field.grids[res][i] *
                                    (self.model.field.masks[res][t] >= 0))
                                t+=1

                self.model.field.use_mask = False
                del self.model.field.masks
            # self.validate()
            self.train_finish()
    ...
    def train(self, args):
        """Override this if some very specific training procedure is needed."""
         # add s3im loss in training process
        self.s3im_weight = args.s3im_weight
        if args.s3im_weight > 0:
            self.s3im_func = S3IM(kernel_size=args.s3im_kernel, stride=args.s3im_stride, repeat_time=args.s3im_repeat_time, patch_height=args.s3im_patch_height, patch_width=args.s3im_patch_width).to(self.device)
    ...
    def train_finish(self):
        grid, space, space_time, non_grid = kplane_param_count(self.model.field)
        grid_bytes = grid * 4
        space_bytes = space * 4
        space_time_bytes  = space_time * 4
        non_grid_bytes = non_grid * 2  # netwrok params use torch.float16 type
        print(f'total params: {(grid_bytes + non_grid_bytes)/1_048_576:.3f}MB '
              f'(Space Grid params: {space_bytes/1_048_576:.3f}MB) '
              f'(Space-Time Grid params: {space_time_bytes/1_048_576:.3f}MB) '
              f'(Non-Grid params: {non_grid_bytes/1_048_576:3f}MB)')

        flat_mask = torch.cat([
            torch.cat([
                p[0].flatten(), p[1].flatten(), p[2].flatten(), p[3].flatten(),
                p[4].flatten(), p[5].flatten()
            ]) for p in list(self.model.field.grids)
        ])
        ...
        #         f'(Grid): {grid_bytes/1_048_576:.3f}MB) '
        #         f'(Non-Grid: {non_grid_bytes/1_048_576:3f}MB)')
        self.save_model()
    ...
    def _normalize_err(self, preds: torch.Tensor,
    ...
    def _normalize_depth(self, depth: torch.Tensor, img_h: int,
                         img_w: int) -> torch.Tensor:
        return (self._normalize_01(depth)).cpu().reshape(img_h, img_w)[...,
                                                                       None]
    ...
    def evaluate_metrics(
        ...
        for k in preds.keys():
            if "depth" in k:
                ...
                out_depth = torch.cat(
                    (out_depth,
                     prop_depth)) if out_depth is not None else prop_depth
        ...
        if gt is not None:
            ...
            out_img = torch.cat((out_img, self._normalize_err(preds_rgb, gt)),
                                dim=0)
        ...
        if save_outputs:
            ...
            if name is not None and name != "":
                out_name += "-" + name
            write_png(os.path.join(self.log_dir, out_name + ".png"),
                      out_img_np)
            if out_depth is not None:
                depth_name = out_name + "-depth"
                write_png(os.path.join(self.log_dir, depth_name + ".png"),
                          out_depth_np)
    ...
    def report_test_metrics(self, scene_metrics: Dict[str, Sequence[float]],
        ...
        for k in scene_metrics:
            ak = f"{k}_{extra_name}"
            scene_metrics_agg[ak] = np.mean(np.asarray(
                scene_metrics[k])).item()
            log_text += f" | {k}: {scene_metrics_agg[ak]:.4f}"
            self.writer.add_scalar(f"test/{ak}", scene_metrics_agg[ak],
                                   self.global_step)
    ...
    def init_lr_scheduler(
            self, **kwargs) -> Optional[torch.optim.lr_scheduler._LRScheduler]:
        eta_min = 0
        lr_sched = None
        ...
        if scheduler_type == "cosine":
            ...
            lr_sched = get_step_schedule_with_warmup(self.optimizer,
                                                     milestones=[
                                                         max_steps // 2,
                                                         max_steps * 3 // 4,
                                                         max_steps * 5 // 6,
    ...
    @abc.abstractmethod
    def init_model(self, use_mask, **kwargs) -> torch.nn.Module:
        pass

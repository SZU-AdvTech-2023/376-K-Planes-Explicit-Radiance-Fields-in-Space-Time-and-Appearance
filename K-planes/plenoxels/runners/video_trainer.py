...
class VideoTrainer(BaseTrainer):
    ...
    def __init__(self, use_mask, mask_weight,
                 tr_loader: torch.utils.data.DataLoader,
                 tr_dset: torch.utils.data.TensorDataset,
                 ts_dset: torch.utils.data.TensorDataset, num_steps: int,
                 logdir: str, expname: str, train_fp16: bool, save_every: int,
                 valid_every: int, save_outputs: bool, isg_step: int,
                 ist_step: int, device: Union[str, torch.device], **kwargs):
    ...
    def validate(self):
        ...
        for img_idx, data in enumerate(dataset):
            ...
            pb.set_postfix_str(f"PSNR={out_metrics['psnr']:.2f}",
                               refresh=False)
            pb.update(1)
        pb.close()
        ...
        if self.save_video:
            ...
            if len(out_depths) > 0:
                write_video_to_file(
                    os.path.join(self.log_dir, f"step{self.global_step}"),
                    out_depths)
        ...
        df.to_csv(
            os.path.join(self.log_dir,
                         f"test_metrics_step{self.global_step}.csv"))
    ...
    def init_model(self, use_mask, **kwargs) -> LowrankModel:
        return initialize_model(self, use_mask, **kwargs)
    ...
    def get_regularizers(self, **kwargs):
        ...
        if not self.model.use_occ_grid:  # proposal network
            losses += [
                PlaneTV(kwargs.get('plane_tv_weight_proposal_net', 0.0),
                        what='proposal_network'),
                L1TimePlanes(kwargs.get('l1_time_planes_proposal_net', 0.0),
                             what='proposal_network'),
                TimeSmoothness(kwargs.get(
                    'time_smoothness_weight_proposal_net', 0.0),
                               what='proposal_network'),
...
def init_ts_data(data_dir, split, **kwargs):
    ...
    downsample = 1.0 # D-NeRF 全分辨率 800*800
